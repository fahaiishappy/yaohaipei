...
class Index extends Common{
    ...
    public function updatemember($id)
    {
        ...
        if($member){
        	if($member['bind_id']){
        		...
        		// if($member['card_id'] == 0){
	            //    	db('member')->where(['id' => $id])->update(['coupon' => 1]);
	            //    	$member['coupon'] = 1;
	            //    }
    ...
    // 获取红人详情
    public function onecewebrity()
    {
        if($this->request->isPost()){
            $data   = input('post.');
            $_list = db('cewebrity')->where('id = '.$data['id'])->find();
            if(isset($data['realmid']) && !empty($data['realmid'])){
                $realm = explode(',',$data['realmid']);
    ...
    {
        $where['status'] = array('eq',1);
        $tablewhere['status'] = array('eq',1);
        $tablewhere['pid'] = array('eq',0);
        $table = db('realm')->where($tablewhere)->order('id asc')->select();
        $_list = db('realm')->where($where)->order('id asc')->select();
        if($this->request->isPost()){
            $data   = input('post.');
            if(isset($data['realm']) && !empty($data['realm'])){
                $data['realm'] = explode(',',$data['realm']);
                foreach ($_list as $key=>$value){
                    if (in_array($_list[$key]['id'], $data['realm'])){
    ...
    {
        $_list = db('banner')->where('status = 1')->order('sort asc')->select();
        return $this->app_json([
            'list' => $_list
        ]);
    }
    ...
    public function articlehits()
    {
        ...
        foreach ($article as $k => $v){
            db('college_article')->where([
                ...
                'views'   => $article[$k]['views']+mt_rand(0,1),
            ]);
        }
    }
    ...
    { 
        if($this->request->isPost()){
            $data   = input('post.');
            $accessToken = get_access_token_gzh();
            $url="https://api.weixin.qq.com/cgi-bin/user/info?access_token=".$accessToken['access_token']."&openid=".$data['openid']."&lang=zh_CN";
    ...
    public function receive()
    {
        if($this->request->isPost()){
            $data   = input('post.');
            $_info = db('member')->where('id='.$data['id'])->find();
            if($_info){
                if($_info['couponmoney'] > 0){
                    return $this->app_json([
                        'code' => 0,
                        'msg'  => '您当前有未使用的优惠券，不可重复领取！',
                    ]);
                }else{
                    $coupon = db('member')->update([
                        'id'         => $_info['id'],
                        'coupon'     => 1,
                        'couponmoney'=> 100,
                        'couponendtime'=> strtotime(date('Y-m-d H:i:s',strtotime('+3 day'))),
                    ]);
                    if($coupon){
                        return $this->app_json([
                            'code' => 200,
    ...
    public function ceshi(){
        // $qrcode = showqrcode();
        // // echo "<img src='https://mp.weixin.qq.com/cgi-bin/showqrcode?ticket=" . $qrcode['ticket'] . "'>";
        // if($qrcode){
        //     $qr = "https://mp.weixin.qq.com/cgi-bin/showqrcode?ticket=" . $qrcode['ticket'] . "";
        //     return $this->app_json([
        //         'code' => 200,
        //         'qrcode'  => $qr,
        //     ]);
        ...
        foreach ($list as $k => $v){
            $urls = array(
                'https://www.hongrensutui.com/details/'.$v['id'].'.html'
            );
            $api = 'http://data.zz.baidu.com/urls?site=https://www.hongrensutui.com&token=qaXSVONls5bE1mfC';
            $ch = curl_init();
            $options =  array(
                CURLOPT_URL => $api,
                CURLOPT_POST => true,
    ...
    {
        //-------配置
        $AppID = 'wx0f07681864173d5c';
        $AppSecret = '992d78b9ccdccd3233266cef569d128d';
        $callback  =  'https://www.hongrensutui.com/api/index/weixin'; //回调地址
        //微信登录
        session_start();
        //-------生成唯一随机串防CSRF攻击
